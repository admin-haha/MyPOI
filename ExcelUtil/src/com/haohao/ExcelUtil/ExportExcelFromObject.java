 ...
 * 
 */
public class ExportExcelFromObject {
	...
	public void exportExcel(String fileName, List beans) throws Exception {
		...
		if (workBook != null) {
			...
			for (int i = 0; i < beanCount; i++) {
				...
				// 将要导出的字段值添加到row中
				fillRowFromObject(row, beans.get(i));
			}
	...
	private void fillRowFromObject(Row row, Object o) throws Exception {
		// 要导出的对象的相应字段名
		Set<String> set = this.mapping.keySet();
		this.index = 0; // 用于创建单元格
		...
		for (String key : set) {
			...
			if (type.equals(Date.class)) {
				row.createCell(index).setCellValue(new SimpleDateFormat(this.datePattern).format(value));
			} else {
